...
export default function HomePage() {
  ...
  const fetchFeaturedContent = async () => {
    try {
      ...
      // Fetch institutes
      const institutesResponse = await fetch('/api/institutes');
      const institutesData = await institutesResponse.json();
      
      // For now, we'll take the first few items as featured
      // In a real implementation, you might have a "featured" flag in the database
      setFeaturedVendors(vendorsData.vendors.slice(0, 3));
      setNearbyInstitutes(institutesData.institutes.slice(0, 3));
      setLoading(false);
    } catch (error) {
  ...
  return (
    <>
      <Head>
        <title>{`${t('site.title', { defaultValue: 'HalalCentral' })} - ${t('site.tagline', { defaultValue: 'Halal Directory' })}`}</title>
        <meta name="description" content={t('site.description', { defaultValue: 'Find halal restaurants, mosques, and community services in Sweden' })} />
